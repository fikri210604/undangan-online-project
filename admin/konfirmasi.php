                ...
                <div class="col-md-12">
                    <form method="GET" class="d-flex flex-wrap gap-2">
                        ...
                        <select name="filter_status" class="form-select form-select-sm me-2" style="width: auto;">
                            ...
                            <option value="tidak hadir" <?= $filter_status == 'tidak hadir' ? 'selected' : '' ?>>Tidak
                                Hadir</option>
                            <option value="belum" <?= $filter_status == 'belum' ? 'selected' : '' ?>>Belum Konfirmasi
                            </option>
                        </select>
                    ...
                    <tbody>
                        <?php if (mysqli_num_rows($query) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($query)):
                                ...
                                $ucapan = $row['ucapan'] ?: '-';
                                $pdf_link = ($pdf !== '-')
                                    ? "<a href='../public/undangan_pdf/$pdf' target='_blank' class='text-danger d-flex align-items-center gap-2 text-decoration-none'>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="crud-tamu/hapus_konfirmasi.php?id=<?= $row['id'] ?>"
                                            class="btn btn-sm btn-danger" onclick="return confirm('Hapus data ini?')">
